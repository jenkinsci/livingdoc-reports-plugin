...
public class LivingDocReportCollector implements FilePath.FileCallable<SummaryBuildReportBean> {
    ...
    public LivingDocReportCollector (FilePath buildDir, int buildId, LivingDocPublisherConfig config) throws IOException,
    ...
    private void publishToConfluence (FilePath xmlReport) {
        ...
        try {
            ...
            PostExecutionResultPublisher pub =
                new PostExecutionResultPublisher(config.getConfluenceUrl(), location, normalizedContent);
    ...
    String getLocation (String filename) {
        ...
        if (location.indexOf(config.getConfluenceSiteTitle()) >= 0) {
            ...
            location.insert(0, "/").insert(0, config.getConfluenceSpaceKey()).insert(0, "-").insert(0,
                config.getConfluenceSiteTitle());
        }
